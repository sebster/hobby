...
@Component
public class JpaPropertyGreaterThanValueSpecificationAdapter<T> extends AbstractJpaPropertySpecificationAdapter<T> {

	@Override
	public boolean canAdapt(Specification<?> spec) {
		return spec instanceof PropertyGreaterThanValue;
	}

	@Override
	public Predicate adapt(Specification<T> spec, Root<? extends T> root, CommonAbstractCriteria crit, CriteriaBuilder cb) {
		PropertyGreaterThanValue<T, ?> gt = cast(spec);
		return cb.gt(toJpaPath(gt.getProperty(), root), gt.getValue());
	}
